...
iptables -A FORWARD -o enp7s0 -i veth1 -j ACCEPT
iptables -A FORWARD -i enp7s0 -o veth1 -j ACCEPT

# masquerade (snat) so that return traffic has somewhere to go
iptables -t nat -A POSTROUTING -s 10.0.3.2/24 -o enp7s0 -j MASQUERADE
